...
export async function fetchSpeckleElements(
  ...
  // Типы объектов, которые НЕ являются конструктивными элементами
  const EXCLUDED_NAMES = [
    'BooleanPart',        // Вырезы, отверстия
    'Fitting',            // Подгонки
    'Weld',               // Сварные швы
    'Bolt',               // Болты
    'BoltGroup',          // Группы болтов
    'SingleRebar',        // Арматура
    'RebarGroup',         // Группы арматуры
    'RebarMesh',          // Сетки
    'SurfaceTreatment',   // Обработка поверхности
    'PourBreak',          // Разрыв бетонирования
    'PourObject',         // Участок бетонирования
  ];
  ...
  for (const obj of rawObjects) {
    ...
    // Пропускаем вспомогательные объекты по имени
  ...
  console.log(`📊 Parsed ${rawObjects.length} objects -> ${elements.length} elements (skipped ${skippedCount} auxiliary)`);
