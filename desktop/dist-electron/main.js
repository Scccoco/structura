...
async function initDatabase() {
  ...
  const wasmPath = require$1.resolve("sql.js/dist/sql-wasm.wasm");
...
ipcMain.handle("folder:scanPDFs", async (_, folderPath) => {
  const results = [];
  function scanDir(dirPath, parentFolder = "") {
    try {
...
ipcMain.handle("db:elements:getByProject", (_, projectId) => {
...
ipcMain.handle("db:elements:updateStatus", (_, guid, status) => {
...
ipcMain.handle("db:elementActs:getByElement", (_, elementGuid) => {
  return queryAll(`
...
ipcMain.handle("db:elementActs:link", (_, elementGuid, actId) => {
...
ipcMain.handle("db:elementActs:unlink", (_, elementGuid, actId) => {
...
ipcMain.handle("db:sync:markSynced", (_, ids) => {
