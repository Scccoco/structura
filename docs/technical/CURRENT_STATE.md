...
*   **Статус**: Код написан (`backend/speckle_adapter`), сервис добавлен в `docker-compose.prod.yml`.
*   **Задача**: Настроить вебхуки.

### Инфраструктура (DevOps)
*   **Auth**: Исправлена проблема с бесконечным редиректом/CORS в Speckle. Добавлены заголовки `X-Forwarded-Proto=https` в `docker-compose.prod.yml`.
*   **SSL**: Traefik автоматически получает сертификаты для всех поддоменов (`app`, `api`, `speckle`, `adapter`, `cloud`, `s3`).
