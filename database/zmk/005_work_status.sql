-- ============================================
-- ZMK Work Status Migration
-- Добавляет статусы работ для сборок
-- ============================================

-- 1. Добавить work_status в assemblies
ALTER TABLE zmk.assemblies 
ADD COLUMN IF NOT EXISTS work_status VARCHAR(50) 
DEFAULT 'model_not_ready';

-- 2. Добавить constraint для допустимых значений
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'assemblies_work_status_check'
    ) THEN
        ALTER TABLE zmk.assemblies 
        ADD CONSTRAINT assemblies_work_status_check 
        CHECK (work_status IN (
            'km_review',         -- Возможны изменения КМ (облака+вопросы)
            'model_not_ready',   -- Модель не готова
            'model_in_progress', -- Модель в работе
            'model_done',        -- Модель разработана
            'kmd_in_progress',   -- КМД в разработке
            'kmd_released',      -- КМД передано в производство
            'in_production',     -- В цехе на изготовлении
            'ready_to_ship',     -- К отгрузке
            'shipped'            -- Отгруженная продукция
        ));
    END IF;
END $$;

-- 3. Создать индекс для быстрой фильтрации по статусу
CREATE INDEX IF NOT EXISTS idx_assemblies_work_status ON zmk.assemblies(work_status);

-- 4. Обновить view v_program с work_status
...
SELECT 'Work status migration complete!' AS status;
